...
figure('units','normalized','outerposition',[.2 .2 .4 .35])
hold on

subplot(1,2,1)
plot(f_test,'-ok','markerfacecolor','k','markeredgecolor','none','linewidth',1)
set(gca,'yscale','log')
...
subplot(1,2,2)
hold on
errorbar(median(diff_mat_2050),SE(diff_mat_2050),'color',c(6,:),'linewidth',1)
errorbar(median(diff_mat_8050),SE(diff_mat_8050),'color',c(2,:),'linewidth',1)
...
ylabel({'Click error in direction of motion (dva)','rel. to equal likelihodd condition'})
yline(0)
