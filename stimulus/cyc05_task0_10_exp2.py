...
ncnds = 17 * 2
# SOA x motionDirection

soa_array_ms = np.repeat(soa_arr_base_ms, 2)
...
for itrial in range(ntrials):
    ...
    for i, icount in enumerate(range(len(motionX_array))):
        for islow in range(slow_coeff):
            ...
            # flash the probe
            if (soa_ms >= 0) & \
                    ((icount == soa_frame) | (icount == soa_frame+1)):
                probe.draw()
                probe_time = my_clock.getTime() * 1000
    ...
    print(f'Flash-BarEnd measured:'
          f' {round(flash2motionEnd_measured_ms)} ms')
    ...
    print(f'probe_xoffset  : {probe_xoffset} dva')
    print(f'probe2bar_distance_ms: {probe2bar_distance_ms} ms')
