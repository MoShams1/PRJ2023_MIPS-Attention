...
# probe-line relation
soa_arr_base_ms = np.arange(-700, 700 + 1, 50)
...
soa_array_ms = np.repeat(soa_arr_base_ms, 2 * 2)
probe_xoffset_array_dva = np.tile(np.repeat(probe_xoffset_base_dva, 2), 29)
motion_dir_array = np.tile(motion_dir_base, 29 * 2)

rep_per_cnd = 2
soa_array_ms = np.repeat(soa_array_ms, rep_per_cnd)
...
for itrial in range(ntrials):
    ...
    if soa_ms < 0:
        for j in range(soa_frame):
            if j in [0, 1]:
                if j == 0:
                    probe_start_time = my_clock.getTime() * 1000
                probe.draw()
            win.flip()
            if j == 1:
                probe_end_time = my_clock.getTime() * 1000
    ...
    for i, icount in enumerate(range(len(motionX_array))):
        for islow in range(slow_coeff):
            ...
            # flash the probe
            if (soa_ms >= 0) & \
                    ((icount == soa_frame) or (icount == soa_frame + 1)):
                if icount == soa_frame:
                    probe_start_time = my_clock.getTime() * 1000
                probe.draw()
            ...
            if (soa_ms >= 0) & (icount == soa_frame + 1):
                probe_end_time = my_clock.getTime() * 1000

    motion_end_ms = my_clock.getTime() * 1000

    motion_dur_measured_ms = motion_end_ms - motion_start_time
    soa_measured_ms = probe_start_time - motion_start_time
    flash2motionEnd_measured_ms = motion_end_ms - probe_start_time
    flash_duration_measured = probe_end_time - probe_start_time

    print(f'SOA: {soa_ms} ms\n')
    # print(f'SOA measured: {round(soa_measured_ms)} ms')
    print(f'flash_duration: 33 ms')
    print(f'flash_duration_measured: {round(flash_duration_measured)} ms')
