...
Task Procedure:
    ...
    The starting position of the bar was +11 dva from the vertical meridian (
    in direction of motion).
    A probe flashes at -4 dva from the bar, at the time the bar reaches the
    midway of its path.
...
output_name = f"cyc05_exp01_probeNeg4_barStart11_{date}_{time}_{subID}.json"
...
bar_color = 'white'
bar_vel = 10  # dva/s
bar_xstart_base = np.arange(-7, 11+1, 1)
bar_ystart = 4  # dva
bar_yend = 4  # dva
motion_dur_ms = 500
...
probe2bar_array_dva = np.repeat(probe2bar_base_dva, 19 * 2)
bar_xstart_array = np.tile(np.repeat(bar_xstart_base, 2), 1)
motion_dir_array = np.tile(motion_dir_base, 1 * 19)
