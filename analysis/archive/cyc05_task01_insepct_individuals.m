...
file_dir = dir('../data/cyc05/*task0_1*');
nsub = numel(file_dir);
...
bar_vel = cell2mat(struct2cell(jsonData.motion_vel_measured));
...
soa_base = unique(soa)';
probe2bar_base = unique(probe2bar)';

soa_count = 0;
for isoa = soa_base
    soa_count = soa_count+1;
    p2b_count = 0;
    for ip2b = probe2bar_base(1:3)
        p2b_count = p2b_count+1;
        ind = (soa == isoa) & (probe2bar == ip2b);
        err_mat(p2b_count, soa_count) = median(click_xerr(ind));
    end
end

figure
plot(soa_base, err_mat', ...
    'linewidth',2)
xlabel 'bar-flash SOA (ms)'
ylabel 'position shift in direction of motion (dva)'
legend({'bar2probe: -250','bar2probe: +250','bar2probe: +500'})
cleanplot

%%
% probe_xoffset_base = unique(probe_xoffset)';
soa_base = unique(soa)';
probe_xoffset_base = unique(probe_xoffset)';
...
for isoa = soa_base
    soa_count = soa_count+1;
    x_count = 0;
    for ix = probe_xoffset_base(1:3)
        x_count = x_count+1;
        ind = (soa == isoa) & (probe_xoffset == ix) & (probe2bar == 250);
        err_mat(x_count, soa_count) = mean(click_xerr(ind));
    end
end
...
xlabel 'bar-flash SOA (ms)'
ylabel 'position shift in direction of motion (dva)'
legend({'probe offset: -1','probe offset: 0','probe offset: +1'})
cleanplot
