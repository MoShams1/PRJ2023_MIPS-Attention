...
for isubplot in range(5):
    ...
    line_passed, = ax[isubplot].plot(probex_arr,
                                     err_mat_passed.mean(axis=0),
                                     color='black')
    ax[isubplot].plot(np.tile(probex_arr, err_mat_passed.shape[0]),
                      err_mat_passed.flatten(),
                      'o', mec='black', mfc='none')
    line_reversed, = ax[isubplot].plot(probex_arr,
                                       err_mat_reversed.mean(axis=0),
                                       color='red')
    ax[isubplot].plot(np.tile(probex_arr, err_mat_reversed.shape[0]),
                      err_mat_reversed.flatten(),
                      'o', mec='red', mfc='none')
    ax[isubplot].set(xticks=probex_arr,
                     xlim=[np.min(probex_arr) - 2, np.max(probex_arr) + 2],
