...
file_dir = dir('../data/cyc05/*task0_8*');
nsub = numel(file_dir);
...
for isoa = soa_base
    ...
    ind = soa == isoa;    
    err_mat1(:,soa_count) = click_xerr(ind);
end
err_mat = [err_mat1(:,1:end-1), nan(8,1), err_mat1(:,end)];

figure('units','inches','outerposition',[0, 0, 5, 5])
x = [soa_base(1:end-1), nan, 300];
y = mean(err_mat);
e = SE(err_mat);
errorbar(x, y, e, ...
    'o-','linewidth',2)
...
for isoa = soa_base
    ...
    ind_inward1 = (soa == isoa) & (probe_pos(:,1) < 0) & (bar_dir > 0);
    ind_inward2 = (soa == isoa) & (probe_pos(:,1) > 0) & (bar_dir < 0);
    ind_inward = ind_inward1 | ind_inward2;
    err_mat2_in(:,soa_count) = click_xerr(ind_inward);

    ind_outward1 = (soa == isoa) & (probe_pos(:,1) < 0) & (bar_dir < 0);
    ind_outward2 = (soa == isoa) & (probe_pos(:,1) > 0) & (bar_dir > 0);
    ind_outward = ind_outward1 | ind_outward2;
    err_mat2_out(:,soa_count) = click_xerr(ind_outward);
end

figure('units','inches','outerposition',[0, 0, 5, 5])
x = [soa_base(1:end-1), nan, 300];

err_mat_out = [err_mat2_out(:,1:end-1), nan(4,1), err_mat2_out(:,end)];
y_out = median(err_mat_out);
e_out = SE(err_mat_out);
errorbar(x, y_out, e_out, ...
    'o-', 'linewidth',2, 'color',.5*ones(1,3))

hold on

err_mat_in = [err_mat2_in(:,1:end-1), nan(4,1), err_mat2_in(:,end)];
y_in = median(err_mat_in);
e_in = SE(err_mat_in);
errorbar(x, y_in, e_in, ...
    '-o', 'linewidth',2, 'color',cmap(2,:))
