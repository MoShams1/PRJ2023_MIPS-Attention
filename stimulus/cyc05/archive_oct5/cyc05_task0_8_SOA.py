...
Task Procedure:
    ...
    A second set of bars moved orthogonal to the first bar inward (100 ms).
...
subID = 'MS'  # subject ID (put 'test' for a test run)
...
output_name = f"cyc05_task0_8_{date}_{time}_{subID}.json"
...
vline2_1 = visual.Rect(win=win,
                       size=(line_width, vline_length / 2),
                       fillColor=line_color,
                       ori=90)
vline2_2 = visual.Rect(win=win,
                       size=(line_width, vline_length / 2),
                       fillColor=line_color,
                       ori=90)
...
for itrial in range(ntrials):
    ...
    line2_1_motion_array = [line_start_ypos,
                            line_start_ypos - xshift_steps,
                            line_start_ypos - 2 * xshift_steps]
    line2_1_motion_array = np.flip(np.repeat(line2_1_motion_array, 2))

    line2_2_motion_array = [line_start_ypos,
                            line_start_ypos + xshift_steps,
                            line_start_ypos + 2 * xshift_steps]
    line2_2_motion_array = np.flip(np.repeat(line2_2_motion_array, 2))
    ...
    bar2_dur_frames = len(line2_1_motion_array)
    if bar12soa_ms >= 0:
        full_dur_frames = bar1_dur_frames
    else:
        full_dur_frames = bar1_dur_frames + bar12soa_frame

    # full_trial_dur_frames = bar1_dur_frames - \
    #                         (bar2_frames * np.sign(bar12soa_ms))
    # full_trial_frames = np.array(range(full_trial_dur_frames))

    # if bar12soa_ms < 0:
    #     bar2show_array = np.ones(bar2_dur_frames)
    #
    # bar1show_arr = np.ones(bar1_dur_frames)
    # bar2show_arr = np.ones(bar2_dur_frames)
    #
    # if bar12soa_ms > 0:
    #
    ...
    for ifull in range(full_dur_frames):
        for islow in range(slow_coeff):
            ...
            if (ibar2 >= 0) & (ibar2 < 6) & (bar12soa_ms != 999):
                if ibar2 == 0:
                    line2_start_time = my_clock.getTime() * 1000
                vline2_1.pos = [line2_start_xpos,
                                line2_1_motion_array[ibar2]]
                vline2_2.pos = [line2_start_xpos,
                                line2_2_motion_array[ibar2]]
                vline2_1.draw()
                vline2_2.draw()
