...
Task Procedure:
    ...
    The starting position of the bar was -2.5 dva from the vertical
    meridian.
    A probe flashes at -1 dva to +2 dva from the bar, at the time the bar
...
output_name = f"cyc05_exp01_highSpatialResolution_{date}_{time}_{subID}.json"
...
probe_rad = .3
probe_color = 'red'
probe2bar_base_dva = np.arange(-1, 2 + .01, .1)
probe_y = 4
...
ncnds = 31 * 1 * 2
# probe2bar x offset x motionDirection

probe2bar_array_dva = np.repeat(probe2bar_base_dva, 1 * 2)
bar_xstart_array = np.tile(np.repeat(bar_xstart_base, 2), 31)
motion_dir_array = np.tile(motion_dir_base, 31 * 1)
...
# probe
probeK = visual.Circle(win,
                       radius=probe_rad * 1.25,
                       fillColor='black')
