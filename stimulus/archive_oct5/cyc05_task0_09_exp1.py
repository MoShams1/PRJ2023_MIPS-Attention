...
output_name = f"cyc05_task0_09_exp1_{date}_{time}_{subID}.json"
...
for itrial in range(ntrials):
    ...
    probe.pos = [(probe2bar_dva + line_xoffset) * motion_dir, probe_yoffset]
    ...
    my_clock.reset()
    probe_time = np.nan
    motion_dur_frames = len(motionX_array)
    halfway_frame = motion_dur_frames / 2
    motion_start_time = my_clock.getTime() * 1000
    for i, icount in enumerate(range(motion_dur_frames)):
